...
/* Landing — the marketing page, ported 1:1 from the Claude Design v2 handoff
   (docs/claude-design/v2/Landing.dc.html). Standalone full page: its own top
   nav, hero, sections and footer — it does NOT use the app <Layout>.
   ...
   Responsive: the design's own r-* CSS system governs the breakpoints
   (960 / 760 / 520). The @media block below is injected verbatim and the
   r-* classNames are attached to the same grid elements as the design —
   NOT Tailwind responsive grid classes. */
...
/* ---------- integrations (trust strip icon tiles) ---------- */
const trustIconSvg = (d: string) =>
  `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" width="16" height="16"><path stroke-linecap="round" stroke-linejoin="round" d="${d}"/></svg>`;
...
/* ---------- pricing plans (raw monthly; price computed by billing) ---------- */
...
const RAW_PLANS: RawPlan[] = [
  {
    name: 'בסיס',
    monthly: 149,
    tagline: 'לעסק שרק מתחיל להאיץ',
    featured: false,
    features: ['3 סוכנים לבחירה', 'מוח מרכזי + CRM', 'עד 1,000 אירועים בחודש', 'אינבוקס מאוחד', 'תמיכה בצ׳אט'],
    cardStyle: {
      position: 'relative',
      padding: '32px 28px',
      borderRadius: 18,
  ...
  },
  {
    ...
    features: [
      'כל הסוכנים בספרייה',
      'פיד אירועים חי + אישורים',
      'עד 10,000 אירועים בחודש',
      'עוזר AI מלא',
      'דוחות ותזרים',
      'תמיכה מועדפת',
    ],
...
const CASE_STUDIES: CaseStudy[] = [
  {
    ...
    tint: '#0e8ba0',
    name: 'סטודיו מיכל',
    role: 'עיצוב פנים · תל אביב',
    challenge: 'בילתה שעות בהעתקת לידים מאינסטגרם לאקסל, ואיבדה פניות.',
    quote: 'תוך שבוע ה‑CRM התמלא לבד. הפסקתי לרדוף אחרי לידים — הם מופיעים מסודרים.',
    stats: [
      { v: '+38%', l: 'לידים שנסגרו' },
      { v: '11 שע׳', l: 'נחסכו בשבוע' },
      { v: '0', l: 'הזנה ידנית' },
    ],
  },
  {
    ...
    name: 'מוסך דהן ובניו',
    role: 'רכב · חיפה',
    challenge: 'פספס לקוחות שכתבו בוואטסאפ בערב ובסופ״ש כשאף אחד לא ענה.',
    quote: 'סוכן הוואטסאפ עונה ללקוחות בלילה במקומי. חסך לי עובד שלם.',
    stats: [
      { v: '24/7', l: 'מענה אוטומטי' },
      { v: '×2.4', l: 'תורים שנקבעו' },
      { v: '4 דק׳', l: 'זמן תגובה' },
    ],
  },
];
...
const FAQ_DATA: Faq[] = [
  ...
  },
  {
    ...
    answer: 'כן. המנוי חודשי, ללא התחייבות. מבטלים בלחיצה ומפסיקים לשלם מהחודש הבא. בחיוב שנתי מקבלים חודשיים חינם.',
  },
  {
  ...
  },
  {
    ...
    answer:
      'רוב העסקים מחברים ערוצים ומפעילים סוכן ראשון תוך פחות מ‑15 דקות, ורואים לידים ואירועים נכנסים לפיד עוד באותו יום.',
  },
  {
    question: 'אילו ערוצים אפשר לחבר?',
    answer:
      'וואטסאפ Business, אינסטגרם, פייסבוק, מייל (Gmail/Outlook), החנות (WooCommerce/Shopify), יומן Google, ומערכות סליקה כמו Grow ו‑Stripe — הכל בלחיצה.',
  },
  {
    question: 'מה קורה כשסוכן רוצה לעשות פעולה רגישה?',
    answer:
      'הוא עוצר ומבקש ממך אישור. שליחת הצעת מחיר, זיכוי או מענה ללקוח כועס — כלום לא יוצא בלי אישורך, ישירות מהמסך או מהנייד.',
  },
  {
    question: 'יש התחייבות או דמי הקמה?',
    answer: 'אין דמי הקמה ואין התחייבות. מתחילים בניסיון חינם ללא כרטיס אשראי, ומשדרגים רק כשמוכנים.',
  },
];
...
/* ---------- responsive system (verbatim from the design's @media block) + animations ---------- */
const LANDING_CSS = `
...
export default function Landing() {
  const [faqOpen, setFaqOpen] = useState<boolean[]>([true, false, false, false, false, false, false, false]);
  ...
  const priceOf = (m: number) => (yearly ? Math.round((m * 10) / 12) : m);
  const billNote = yearly ? 'בחיוב שנתי · חודשיים חינם' : 'בחיוב חודשי · ללא התחייבות';
  ...
  return (
    <div dir="rtl" lang="he" style={{ minHeight: '100vh', background: '#f5f7fb', color: '#0f172a', overflowX: 'hidden' }}>
      ...
      >
        <nav
          className="r-main px-4 py-3 md:py-[14px]"
          style={{
            maxWidth: 1280,
            margin: '0 auto',
            paddingInline: 32,
      ...
      <section style={{ maxWidth: 1280, margin: '0 auto', padding: '20px 32px 24px' }}>
        <div style={{ textAlign: 'center', fontSize: 12, letterSpacing: '.14em', color: '#94a3b8', marginBottom: 20 }}>
          מחובר לכל הערוצים שכבר יש לך — בלחיצה אחת
        </div>
        <div style={{ display: 'flex', justifyContent: 'center', gap: 12, flexWrap: 'wrap' }}>
          {INTEGRATIONS.map((i) => (
            ...
            >
              ...
              />
              <span style={{ fontSize: 14, fontWeight: 600, color: '#334155' }}>{i.name}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="agents" style={{ maxWidth: 1280, margin: '0 auto', padding: '72px 32px' }}>
        ...
        <div className="r-3" style={{ marginTop: 40, display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 20 }}>
          {AGENTS.map((a) => (
            <div
              key={a.title}
      ...
      <section id="how" style={{ maxWidth: 1280, margin: '0 auto', padding: '80px 32px' }}>
        ...
        <div className="r-3" style={{ marginTop: 48, display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 24 }}>
          {STEPS.map((s) => (
            <div
              key={s.num}
              style={{
      ...
      <section id="product" style={{ background: '#0e1a2b', color: '#eef4fb' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '84px 32px' }}>
          ...
          <div className="r-4" style={{ marginTop: 44, display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 20 }}>
            {FEATURES.map((f) => (
      ...
      <section id="pricing" style={{ maxWidth: 1280, margin: '0 auto', padding: '84px 32px' }}>
        <div style={{ textAlign: 'center', maxWidth: '34em', margin: '0 auto' }}>
          ...
          <p style={{ margin: '14px 0 0', fontSize: 17, color: '#55627a' }}>
            במקום אלפי שקלים לחברת אוטומציה — מנוי שאפשר לבטל בכל רגע.
          </p>
          <div
            style={{
              marginTop: 24,
          ...
          >
            <button style={yearly ? segOff : segOn} onClick={() => setBilling('monthly')}>
              חודשי
            </button>
            <button style={yearly ? segOn : segOff} onClick={() => setBilling('yearly')}>
              שנתי · חסכו 17%
            </button>
          </div>
        </div>
        <div
          className="r-3"
          style={{ marginTop: 44, display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 22, alignItems: 'start' }}
        >
          {RAW_PLANS.map((p) => (
            <div key={p.name} style={p.cardStyle}>
              ...
              <div style={{ marginTop: 14, display: 'flex', alignItems: 'flex-end', gap: 6 }}>
                <span style={{ fontSize: 18, color: p.mutedColor, marginBottom: 10 }}>₪</span>
                <span
                  className="tabular-nums"
                  style={{
              ...
              <p style={{ margin: '6px 0 0', fontSize: 12.5, color: p.mutedColor }}>{billNote}</p>
              <p style={{ margin: '8px 0 0', fontSize: 14, color: p.mutedColor }}>{p.tagline}</p>
      ...
      <section style={{ background: '#fff', borderBlock: '1px solid rgba(15,23,42,.08)' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '80px 32px' }}>
          ...
          <div className="r-3" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 22 }}>
            {TESTIMONIALS.map((t) => (
              <div
                key={t.name}
      ...
      <section style={{ maxWidth: 1280, margin: '0 auto', padding: '80px 32px' }}>
        <div style={{ textAlign: 'center', maxWidth: '34em', margin: '0 auto 44px' }}>
          <span style={{ fontSize: 12, letterSpacing: '.16em', color: '#0b7688', fontWeight: 600 }}>סיפורי לקוחות</span>
          <h2
            style={{
              margin: '12px 0 0',
              fontFamily: "'Space Grotesk','Heebo',sans-serif",
          ...
          >
            מספרים אמיתיים מעסקים אמיתיים
          </h2>
        </div>
        <div className="r-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 22 }}>
          {CASE_STUDIES.map((c) => (
            <div
              key={c.name}
              className="glass-card"
              style={{ padding: 30, borderRadius: 18, display: 'flex', flexDirection: 'column', gap: 18 }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12.5, color: '#94a3b8' }}>{c.role}</div>
                </div>
              </div>
              <div style={{ borderInlineStart: '3px solid rgba(15,23,42,.1)', paddingInlineStart: 14 }}>
                <div
                  style={{
                    fontSize: 12,
                    fontWeight: 700,
                ...
                <p style={{ margin: 0, fontSize: 14.5, color: '#55627a', lineHeight: 1.6 }}>{c.challenge}</p>
              </div>
              <p style={{ margin: 0, fontSize: 16, color: '#0f172a', lineHeight: 1.65, fontWeight: 500 }}>״{c.quote}״</p>
              <div
                style={{
                  marginTop: 'auto',
                  display: 'grid',
              ...
              >
                {c.stats.map((s) => (
                  <div key={s.l}>
                    ...
                    <div style={{ fontSize: 12, color: '#55627a', marginTop: 2 }}>{s.l}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section style={{ maxWidth: 1280, margin: '0 auto', padding: '0 32px 84px' }}>
        <div
          style={{
            ...
            overflow: 'hidden',
            borderRadius: 26,
            background: 'linear-gradient(150deg,#0f6b7d 0%,#0e8ba0 55%,#12a1b8 100%)',
            padding: '64px 48px',
            textAlign: 'center',
            color: '#fff',
            boxShadow: '0 30px 70px -30px rgba(14,139,160,.55)',
          }}
        >
          <div
            style={{
